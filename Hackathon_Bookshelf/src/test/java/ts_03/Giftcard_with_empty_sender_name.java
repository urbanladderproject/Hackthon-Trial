...
public class Giftcard_with_empty_sender_name extends utils.ExtentReport {
	...
	//Tries to fill an empty value in the sender name field of the form
	@Test
	public void empty_sender_name() throws Exception {
		
		Report report = new Report();

		log = reports.createTest("Test empty sender name");
		...
		report.select(log, "Empty sender name");

		log.pass(MarkupHelper.createLabel("Empty sender name wasn't accepted", ExtentColor.GREEN));
		...
		gc.sender_name("Empty Sender Name");
		...
		Screenshots.takesnap(driver, "Empty sender name",browsertype);

		report.display(log, "Empty sender name wasn't accepted");
	...
	public void afterMethod(ITestResult result) throws Exception {
		...
		if (result.isSuccess())
			excel.reportToExcel("Empty sender name accepting Test: SUCCESS",browsertype);
		else
			excel.reportToExcel("Empty sender name accepting Test: FAILURE",browsertype);
	}

	@AfterClass
	public void closeBrowser() throws IOException {
		excel.reportToExcel("Empty sender name accepting Test: ENDED",browsertype);
		// close the driver
